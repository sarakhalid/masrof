...
class PortalTransaction(CustomerPortal):
    ...
    def _prepare_home_portal_values(self):
        ...
        wallet = request.env['e.wallet'].search([('responsable','=',request.env.user.partner_id.id) ]) 


        transaction_count = request.env['wallet.transactions'].search_count([
            ('state', '=', 'done'),('wallet','=',wallet.id)
        ]) 
    ...
    def portal_my_transactions(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        ...
        AccountTransaction = request.env['wallet.transactions']
        wallet = request.env['e.wallet'].search([('responsable','=',request.env.user.partner_id.id) ])  

        domain = [('state', '=', 'done'),('wallet','=',wallet.id)]
        ...
        archive_groups = self._get_archive_groups('wallet.transactions', domain) if values.get('my_details') else []
        ...
        return request.render("e_wallet.portal_my_transactions", values)
    ...
    def portal_my_transaction_detail(self, transaction_id, access_token=None, report_type=None, download=False, **kw):
        try:
            transaction_sudo = self._document_check_access('wallet.transactions', transaction_id, access_token)
